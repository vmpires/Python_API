...
urllatlon = 'https://nominatim.openstreetmap.org/search/'+urllib.parse.quote(address)+'?format=json'
response = requests.get(urllatlon).json()

weather = requests.get(f"https://api.hgbrasil.com/weather?key={(key['key'])}&lat={(response[0]['lat'])}&lon={(response[0]['lon'])}")
